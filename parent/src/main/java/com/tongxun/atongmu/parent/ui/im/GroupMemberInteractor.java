...
import com.tongxun.atongmu.parent.model.GroupMemberCallback;
...
public class GroupMemberInteractor implements IGroupMemberContract.Interactor {

    @Override
    public void getGroupMember(final onFinishListener listener) {
        String url= Constants.restGetAddressBook;
        OkHttpUtils.postString()
                ...
                .execute(new StringCallback() {
                    ...
                    public void onResponse(String response, int id) {
                        Gson gson=new Gson();
                        GroupMemberCallback callBack= null;
                        try {
                            callBack = gson.fromJson(response,GroupMemberCallback.class);
                        } catch (JsonSyntaxException e) {
                            e.printStackTrace();
                        }
                        if(callBack!=null){
                            if(callBack.getStatus().equals("success")){
                                listener.onSuccess(callBack.getParentdatas(),callBack.getTeacherdatas());
                            }else {
